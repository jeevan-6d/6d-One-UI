...
const DemoTable = () => {
  ...
  // const reOrderRecords = (hoverIndex, dragIndex) => {
  //   hoverIndex = 0;
  //   dragIndex = 1;
  //   ref.current.setDragnDropIndexes(hoverIndex, dragIndex);
  // };

  const getReorderedRecords = () => {
    //console.log(ref.current.getReorderedRecords());
    const returnObj = ref.current.getReorderedRecords();
    setRecordIds(returnObj.reOrderedIds);
  };
  ...
  const ExportComponent = () => {
    ...
    return (
      <div className="dropdown">
        <select className="btn-micro btn-app-secondary btn-export">
        ...
        >
          <div>Export</div>
          <div className="column-hide-svg export-icon" >
            <svg id="Sorting" xmlns="http://www.w3.org/2000/svg" width="15" height="5" viewBox="0 0 6 4">
              <path id="Path_1041" data-name="Path 1041" d="M11,3H5L8,7Z" transform="translate(-5 -3)" fill="#6B6B6B" />
            </svg>
          </div>
        </button>
        {isOpen ? (
          ...
          <span data-tip={true} data-for={"filter"}>
            <button
              className={`btn btn-outline-primary btn-sm ${
                filterFormData ? "filter-active" : " "
              }`}
              type="button"
            >
              <div
                className="column-hide-svg btn-icon-color filter-icon"
      ...
      <div className="row">
        <div className="col-md-12">
          <AppTable
            ...
            // langChangedValues={{ quickSearchTooltip:"Quick Search", refreshTableTooltip:"Refresh Table", hideColumnTooltip:"Hide Columns", searchField: "Quick Search", perPageCount: "Per View", pagination: "Page Number", paginationOf: "of", firstPage: "F", prevPage: "P", nextPageState:"N", lastPage: "L",noDataAvailableTag: "Hey, looks like there is no data to show!!!",  expanderText: "expanderr", selectionText: "selectionn", actionHeader: "Action", loaderText: "Loading..."}}
        ...
        {/* <div className="col-md-4">
          <br />
          <button className="btn btn-primary mb-5" onClick={pullData}>{`Refresh Table`}</button>
          <button className="btn btn-primary mb-5" onClick={getReorderedRecords}>{`Get ReOrdered Details`}</button>
        </div>
      </div>
      <div className="col-12">
        <p className="mb-0">Reordered id's</p>
        {recordIds.length ? recordIds.join(", ") : ""} */}
      </div>
    </React.Fragment>
  );
};
const AUTH_LIB_VIEW_SCOPE = ["scope:viewsdsd"];
const PROTECTED_MSISDN_TABLE_RESOURCE = "res:msisdn-protected-msisdn";

const onLoadComponent = () => {
  return (
    <React.Fragment>
      <h3>I am your onLoadComponent</h3>
    </React.Fragment>
  );
};

const withAuthCallback = () => {
  console.log("I am your callback!!!!!");
  return <DemoTable />;
};
...
// export default withAuth(DemoTable, AUTH_LIB_VIEW_SCOPE, PROTECTED_MSISDN_TABLE_RESOURCE,onLoadComponent, withAuthCallback);
